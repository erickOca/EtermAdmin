...
export class UserProfileComponent implements OnInit {
  ...
  save(user: User) {
if(!user.usuario_nombre || !user.usuario_pwd || !user.cve_status || !user.tipo_usuario){
  this.snackBar.open("Por favor, complete los campos requeridos", "Cerrar", { duration: 3000 });
  return;
}
    this.userService.saveUser(user).subscribe((data) =>{
      this.snackBar.open('Se ha creado un nuevo usuario', 'Cerrar', { duration: 3000,
...
this.userService.deleteUser(usuarioId).subscribe(
  data => {
    this.showListUser();
    this.snackBar.open('Se ha borrado el usuario', 'Cerrar', { duration: 3000, panelClass: 'blue-snackbar'  });
  }
)
}
